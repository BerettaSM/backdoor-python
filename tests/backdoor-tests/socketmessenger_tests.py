...
@pytest.fixture
def socket_messenger() -> SocketMessenger:
...
class TestSocketMessengerRead:
    ...
    def test_read_should_return_bytes_of_len_0_when_len_0_is_sent(
        self, socket_messenger: SocketMessenger, socket_with_message: socket
    ) -> None:

        data = socket_messenger.read(socket_with_message)
...
class TestSocketMessengerSend:
    ...
    def test_send_should_send_payload_size_then_payload(
        self, socket_messenger: SocketMessenger, socket_pair: tuple[socket, socket]
    ) -> None:
        message = "message"
        inp, out = socket_pair

        socket_messenger.send(inp, message.encode())

        try:
            size = int(out.recv(socket_messenger.paddingsize))
            out.recv(size)
        except TimeoutError:
